...
// ── Receipt / Invoice ─────────────────────────────────────────────────────────
let _receiptCounter = Number(localStorage.getItem("avaReceiptNo") || 1000);

function showReceipt(items, store){
  const now       = new Date();
  const receiptNo = ++_receiptCounter;
  localStorage.setItem("avaReceiptNo", receiptNo);

  const user = localStorage.getItem("avaUser") || "Cashier";

  // Fill meta
  document.getElementById("rcptStoreName").textContent = store;
  document.getElementById("rcptNo").textContent        = String(receiptNo).padStart(6, "0");
  document.getElementById("rcptDate").textContent      = now.toLocaleDateString("en-PH");
  document.getElementById("rcptTime").textContent      = now.toLocaleTimeString("en-PH", {hour:"2-digit", minute:"2-digit"});
  document.getElementById("rcptCashier").textContent   = user;
  ...
  tbody.innerHTML = items.map(item => {
    ...
    return `
      <tr class="rcpt-item-row">
        <td>
          <div class="rcpt-item-name">${item.product}</div>
          <div class="rcpt-item-meta">${[item.color, item.size].filter(Boolean).join(" · ")}</div>
        </td>
        <td style="text-align:center;">${item.qty}</td>
        <td style="text-align:right;">₱${item.price.toLocaleString("en-PH", {minimumFractionDigits:2})}</td>
        <td style="text-align:right;font-weight:700;">₱${subtotal.toLocaleString("en-PH", {minimumFractionDigits:2})}</td>
      </tr>
    `;
  }).join("");

  document.getElementById("rcptTotalItems").textContent = totalQty + " pcs";
  document.getElementById("rcptGrandTotal").textContent = "₱" + grandTotal.toLocaleString("en-PH", {minimumFractionDigits:2});
    ...
    <head>
      ...
      <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Courier New',monospace; }
        ...
        .rcpt-logo img { height:40px; }
        .rcpt-store-name { text-align:center; font-size:18px; font-weight:900; color:#1e1b4b; margin-bottom:2px; font-family:Arial; }
        .rcpt-tagline { text-align:center; font-size:11px; color:#6b7280; margin-bottom:12px; }
        .rcpt-divider { border-top:1px dashed #ccc; margin:10px 0; }
        .rcpt-meta { font-size:12px; display:flex; flex-direction:column; gap:4px; margin-bottom:4px; }
        .rcpt-meta div { display:flex; justify-content:space-between; }
        .rcpt-table { width:100%; border-collapse:collapse; font-size:12px; margin:8px 0; }
        .rcpt-table th { border-bottom:1px solid #ccc; padding:4px 0; font-weight:700; }
        .rcpt-table td { padding:5px 0; vertical-align:top; }
        .rcpt-item-name { font-weight:700; font-size:12px; }
        .rcpt-item-meta { font-size:10px; color:#6b7280; }
        .rcpt-totals { font-size:13px; }
        .rcpt-total-row { display:flex; justify-content:space-between; padding:3px 0; }
        .rcpt-grand-total { font-size:16px; font-weight:900; color:#1e1b4b; border-top:2px solid #1e1b4b; margin-top:6px; padding-top:6px; }
        .rcpt-footer { text-align:center; font-size:12px; color:#6b7280; margin:10px 0 4px; }
        .rcpt-barcode-line { text-align:center; letter-spacing:2px; font-size:14px; color:#ccc; }
      </style>
    </head>
...
// If historyCache is empty, fetch it first
if(historyCache.length === 0){
  box.innerHTML = `<div class="report-empty" style="color:#9ca3af;padding:20px;text-align:center;">Loading...</div>`;
  await loadHistoryCache();
}
